...
# Woocommerce get all order details
        ...
        select
            ...
            ( select group_concat( order_item_name separator '|' ) from S619mavp_woocommerce_order_items where order_id = p.ID ) as order_items
        from
            S619mavp_posts p 
            join S619mavp_postmeta pm on p.ID = pm.post_id
            join S619mavp_woocommerce_order_items oi on p.ID = oi.order_id
        where
...
# Woocommerce get all product details. 
        ...
        SELECT 
                ...
                (SELECT post_title FROM S619mavp_posts WHERE id = pm.post_id) AS title,
                (SELECT meta_value FROM S619mavp_postmeta WHERE post_id = pm.post_id AND meta_key = "_price" LIMIT 1) AS price, 
                (SELECT meta_value FROM S619mavp_postmeta WHERE post_id = pm.post_id AND meta_key = "_regular_price" LIMIT 1) AS "regular price", 
                (SELECT meta_value FROM S619mavp_postmeta WHERE post_id = pm.post_id AND meta_key = "_stock" LIMIT 1) AS stock, 
                (SELECT meta_value FROM S619mavp_postmeta WHERE post_id = pm.post_id AND meta_key = "_stock_status" LIMIT 1) AS "stock status", 
                IFNULL((SELECT meta_value FROM S619mavp_postmeta WHERE post_id = pm.post_id AND meta_key = "_sku" LIMIT 1),
                    (SELECT meta_value FROM S619mavp_postmeta WHERE post_id = pm.post_id AND meta_key = "_custom_field" LIMIT 1)) as SKU,
                IFNULL(SUBSTR( (SELECT meta_value FROM S619mavp_postmeta WHERE post_id = pm.post_id AND meta_key = "_product_attributes" LIMIT 1),
                    INSTR((SELECT meta_value FROM S619mavp_postmeta WHERE post_id = pm.post_id AND meta_key = "_product_attributes" LIMIT 1), "is_variation")+16,1),1) AS "variation"

                FROM `S619mavp_postmeta` AS pm
                JOIN S619mavp_posts AS p ON p.ID = pm.post_id
                JOIN S619mavp_term_relationships AS tr ON tr.object_id = IF(p.post_parent = 0, p.ID, p.post_parent)
                JOIN S619mavp_term_taxonomy AS tt ON tt.taxonomy = 'product_cat' AND tt.term_taxonomy_id = tr.term_taxonomy_id 
                JOIN S619mavp_terms AS t ON t.term_id = tt.term_id
